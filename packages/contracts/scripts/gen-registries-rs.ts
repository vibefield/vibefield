...
const lines: string[] = [
  ...
  ...Object.entries(TERMINAL_PIPELINE).map(([k, v]) => `    pub const ${k}: u64 = ${v};`),
  "}",
  "",
  ...
  ...Object.entries(TERMINAL_PIPELINE_CLOSE_CODES).map(
    ([k, v]) => `    pub const ${k}: u16 = ${v};`,
  ),
  "}",
  "",
