...
export async function restrictToCurrentUser(path: string): Promise<void> {
  ...
  const ace = (await isDirectory(path)) ? `${account}:(OI)(CI)F` : `${account}:F`;
...
/** `DOMAIN\user`, or the bare user when no domain is exported. Falls back to
 * the well-known %USERNAME% form; an empty account would make icacls grant
 * nothing and lock the daemon out of its own tree, so this refuses instead. */
function currentAccount(): string {
  const user = process.env["USERNAME"];
  if (user === undefined || user.length === 0) {
    throw new Error("cannot restrict a path to the current user: USERNAME is unset");
  }
