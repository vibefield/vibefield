...
| PRC-D1…PRC-D13 | plugin-architecture §1.2 · `thinking-plugin-runtime-composability.md` §12 | **RATIFIED 2026-08-15** (James: “go ahead”). **PRC-0 shared ownership/target core LANDED `7e0a0e8`** (24/24 focused package tests). **PRC-1 renderer ownership adapter LANDED `4831b0a`**: child-bound contexts, synchronous publication withdrawal, awaited LIFO cleanup, honest same-realm non-quiescence, and generated author guidance are production-tested. PRC-2 service drain, controllers, behavior/revocation wiring, observability, and update lanes remain. Adapter integration must precede lifecycle-heavy W2 behavior work. |
