...
| ESR-1…ESR-15 | electron-shell-refactor §1.1 | ESR COMPLETE as specced (§17). **ESR-8 AMENDED 2026-08-11 (WIN-9):** its CSP clause assumed the renderer's loopback ports were PINNED, which is what let `buildCsp("production")` enumerate `PORTS.FIELDD_WS_CONTROL`/`_DATA`. UA-D12/UA-5 made every pair bind ephemeral, so that policy refused the renderer's own socket and `doc.list` timed out at 8 s — reproducible in production mode on macOS, never Windows-specific. Production now admits the loopback HOST with the port open (the aperture smoke modes already used). The port cannot be named here by construction: the policy is installed before the first window exists (ESP §6.2) while the window deliberately does not wait for the daemon (design-03 §4.3), and recovery/user-switch replace the port under a live document. §5.2.4's "possible before daemon readiness only because production ports are pinned" is corrected at its source in `security-policy.ts` |
...
| WIN-D10 mutual pairing auth | contracts `envelope.ts` (`PairingMac.nonce` · `HelloAck.serverMac`) · `pairing.rs` / `pairing.ts` · pinned by `fixtures/pairing.vector.json` | **ADOPTED 2026-08-11 (WIN-10).** The D8 MAC proves the CLIENT to field-native; nothing proved the SERVER, and on unix nothing had to — the socket sat inside a 0700 run directory, so only the owner could have created the thing answering. WIN-D1's named pipes have no such boundary: the namespace is flat and machine-wide and the scope is a hash of a guessable data root, so another local account can publish our name before field-native does, and fieldd's connect-probe reads that squatter as a live native (so no real one is spawned). Its ack could then name its own `terminal` control/frame endpoints and auth token. The client now sends a per-connection `nonce` and the server answers `serverMac = HMAC(secret, "fn-ack" 0x00 nonce 0x00 bootId)` — a DIFFERENT context from the client's `fn-boot`, so neither direction's transcript replays as the other's. Both mgmt and the meshdata byte lane carry it (the lane holds no token, but a squatter there feeds fieldd forged document bytes). **The field is optional on the wire and MANDATORY in the client, deliberately:** tolerating absence is precisely the downgrade an attacker would request. Cost, stated honestly: a field-native predating WIN-10 must be restarted once before a newer fieldd will pair with it, and the refusal says so. Does NOT address the DoS leg — a squatter holding the name makes field-native's `first_pipe_instance` bind fail closed, which is honest refusal rather than compromise |
