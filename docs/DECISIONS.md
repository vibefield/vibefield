...
| PRC-D14 / PRC-5 delivery state | plugin-architecture §1.2/§18.5 · `thinking-prc5-update-coordinator.md` | **PRC-5 COMPLETE 2026-08-17.** PRC-D14 freezes exact participant incarnations at synchronous ingress close, distinguishes host-drained leave, positive boundary death, and mere disconnect, and holds newcomers outside the vote. E18/E13/E19/E20 are 7/7, 6/6, 5/5, and 10/10. **PRC-5a `c5dcca9`:** host identity and strict identity-free acknowledgements. **PRC-5b `6817116`:** immutable full-artifact slots and fail-closed CAS pointer. **PRC-5c `3fa514f` + `1602654`:** explicit candidate module/service authority. **PRC-5d `6cafd0a`:** stable renderer facade with break-before-make replacement and pre-commit-only old recovery. **PRC-5e `7873e60` + `31a48c1` + `303374f` + `ffd1ae2` + `0c58fe7`:** coordinator, authenticated source/ack transport, exact inverse, activated install/service/renderer assembly, and held newcomers. **PRC-5f `b35f73d`:** bounded phase/death deadlines; Electron request acceptance remains distinct from exact token-correlated process-gone evidence. **PRC-5g `137f086`:** the v2 pointer atomically pairs artifact and monotonic commit epoch; pre/post-rename child SIGKILL reconstructs old/1 or candidate/2; post-rename errors fail publication-indeterminate; owned/adopted daemon recovery and a boot-id fence replace every surviving renderer realm. The packaged two-window witness preserved 22 widgets/5 staged plugins in both windows, stable participant ids, fresh incarnations, and a new boot id. Experiments also fixed concurrent module-token generation and the Unix smoke-root `sun_path` overflow. Final gates: fieldd 566 passed + 1 platform skip, Electron 574/574, supervisor 54/54, focused PRC-5g 59/59, and physical two-window smoke. Literal hardware power loss and a Windows filesystem run remain named release/portability witnesses, not claimed local evidence. **NOW: PRC-6** bounded diagnostics and soak. |
| PRC-6 delivery state | plugin-architecture §23/§24.4/§24.6 · `thinking-prc6-diagnostics-soak.md` | **IN FLIGHT; PRC-6a LANDED `26fcfdf`.** The old per-scope recursive cap emitted >1,000 rows/>256 KiB under an advertised limit of 8; a 64-row open-object history exceeded 128 KiB; and 2,000 nested scopes overflowed the stack. Evidence revised repeated ancestry paths to parent-indexed flat rows. The shared runtime now hard-caps the complete projection at 128 effects/32 errors, compacts lifecycle history to 64 typed events with explicit loss, retains one bounded close report and exact last boundary-force outcome, exposes loading/blocked scopes, contains passive observer faults, and refuses ownership level 65. Options may reduce but not enlarge ceilings. Forced-GC/plain-wire controls pass 3/3; runtime 50/50; 2,388 live effects measured 1.29 ms p99 and 9,556 measured 8.34 ms p99, both 9,829 bytes. **NOW: PRC-6b** passive host aggregation + lifecycle logs; PRC-6c owns clean/leak oracles and the literal 24-hour physical soak. |
