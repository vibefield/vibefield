...
| PRC-4 delivery state | plugin-architecture §8.8/§12.7 · `thinking-prc4-behavior-adapter.md` | **PRC-4b/4c consumed:** ICE 0.8.1 + strata 0.13.0 carry I16/I17/I18 and pass the installed **10/10** control matrix. **PRC-4d LANDED `95da1b8` / E19:** strict signed behavior descriptors/riders, verbatim projection, exact sealed renderer binding, effective-grant authorization, dormant widget construction, authoring/build surfaces, and activation-root withdrawal. **PRC-4e LANDED `6289795` / E20:** identity-bound window catalog, synchronous canonical document-generation host, bounded chronic ledger, rollback, presence coeffect, structured diagnostics, and reverse teardown across normal and quarantined-reset paths are production-tested. **PRC-4f LANDED `9fc2de6` / E21:** packaged durable/runtime migration, breaker carryover, exact churn cleanup, and 45 balanced registration inverses. **PRC-4g1 CORE CODE LANDED `43929e7` + `81c21fd` / E22:** the ticketed document socket, bounded latest-only room router, shared ids, MeshData barrier, ICE attach order, authenticated QUIC/UDP fragmentation, terminal replay, and receiver STOP/reopen are implemented; the real-tailnet witness link-compiles but still needs its auth-key run. **PRC-4g2 remains closed:** E22 measures one legal 16-facet plugin at 66,498 wire bytes and runtime writes remain unbounded. I19 (or a separately ratified fixed-width profile), aggregate headroom, and a packaged two-engine remote-tombstone witness must pass before the stable refusal lifts; `ce43bad` now tells authors that actual budget gate. |
