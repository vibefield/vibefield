...
| PRC-D14 / PRC-5 delivery state | plugin-architecture §1.2/§18.5 · `thinking-prc5-update-coordinator.md` | **PRC-D14 EVIDENCE AMENDMENT ADDED 2026-08-17:** freeze exact participant incarnations at synchronous ingress close; distinguish host-drained leave, positive boundary death with forward recovery, and mere disconnect; hold newcomers outside the vote. **PRC-E18 GREEN 7/7 + E13 6/6 + E19 5/5 + E20 10/10:** the barrier model covers service + two windows, one commit epoch, ack/recovery barriers, close/crash/disconnect distinctions, stale identity/update refusal, retained external history, no post-commit rollback, and explicit candidate authority. **PRC-5a LANDED `c5dcca9`:** shell-minted stable-window/exact-document identities and strict identity-free acknowledgement contracts. **PRC-5b LANDED `6817116`:** full signed-artifact-SHA immutable slots, durable private staging, fail-closed base-slot CAS pointer publication, legacy adoption, and conservative recovery. **PRC-5c LANDED `3fa514f` + `1602654`:** registry runtime identity is the complete artifact slot; code-free candidate inspection leaves live maps untouched; update-bound path-free module grants resolve the explicit root; the common target controller holds activated candidates at a synchronous stale-checked publication edge; service workers drain old and remain private until the exact candidate row is current. **PRC-5d LANDED `6cafd0a`:** one stable renderer controller/facade owns defensively frozen artifact sources; old quiesces before deferred candidate import; fixed widget-projection changes refuse code-free; candidate module and product-client authority are explicit; strict identity-free prepare/commit/recover acknowledgements preserve private publication across either pointer/command ordering; same-realm non-quiescence permanently cancels import; fresh-URL old recovery is pre-commit only. Production gates: field-app 495/495 (focused 39/39), plugin-runtime 42/42, fieldd candidate/service seams 18/18, Electron seam 6/6, four dependent typechecks, Biome, and patch hygiene. PRC-5e must implement authenticated update-bound candidate-client mint/revocation and source resolution; physical power-loss/Windows durability remains PRC-5g. **NOW: PRC-5e** fieldd coordination, pointer CAS, ack transport, and held-newcomer admission; death witness and physical acceptance follow. |
