...
| Namespace | Home | Status (current through 2026-08-16) |
|---|---|---|
...
| PRC-4 delivery state | plugin-architecture §8.8/§12.7 · `thinking-prc4-behavior-adapter.md` | **PRC-4b/4c consumed:** ICE 0.8.1 + strata 0.13.0 carry I16/I17/I18 and pass the installed **10/10** control matrix. **PRC-4d LANDED `95da1b8` / E19:** strict signed behavior descriptors and riders, verbatim projection, exact sealed renderer binding, effective-grant authorization, dormant widget construction, authoring/build surfaces, and activation-root withdrawal are production-tested. **PRC-4e next:** observable window catalog and synchronous canonical durable/runtime document-generation batch; PRC-4f then owns conformance. Ephemeral admission remains refused until the product document-room presence lane and two-engine tombstone witness exist. |
