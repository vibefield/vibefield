...
installation are fenced against stale observation and backend generations. PRC-3c/3d now bind the
controller to the real service and renderer instances; they must land before W2 depends on
lifecycle behavior.
...
- **PLUG → the mind map pack (ECS-native) + the ECS door** — the chosen thread. P8b and
  ...
  `96fcd81`, PRC-3a `4c4755c`, and PRC-3b `2dbb359`+`22a4f71` are LANDED**; service/renderer
  controller adapters PRC-3c/3d must land before W2 consumes lifecycle semantics. **Gating now: two
