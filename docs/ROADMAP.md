...
proof-carrying force. PRC-3b credential rotation and the service/renderer adapters are next; they
must land before W2 depends on lifecycle behavior.
...
- **PLUG → the mind map pack (ECS-native) + the ECS door** — the chosen thread. P8b and
  ...
  `96fcd81`, and PRC-3a `4c4755c` are LANDED**; PRC-3b credential rotation plus service/renderer
  controller adapters must land before W2 consumes lifecycle semantics. **Gating now: two
