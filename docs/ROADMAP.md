...
`7e0a0e8`: a shared activation-ownership core, exact structured runtime targets, and 24/24 focused
package tests. PRC-1 renderer and PRC-2 service adapters come next; those integrations must land
before W2 depends on lifecycle behavior.
...
- **PLUG → the mind map pack (ECS-native) + the ECS door** — the chosen thread. P8b and
  ...
  builder worktrees. **PRC-D1…D13 are RATIFIED and PRC-0 LANDED `7e0a0e8` (24/24)**;
