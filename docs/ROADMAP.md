...
| UA — users & accounts | **IN FLIGHT** — spec v0.2 RATIFIED 2026-08-05; UA-0 `fcd0af9` · UA-1 `ca1ce49` · UA-2 `3750f20` · **UA-3 link lifecycle + Account page `15832c0`/`30aece8`/`96e9e9d`** (split-hands: core in-checkout, UI by worktree agent; live S1 probe in the suite); owed to the next `pnpm dev`: live dev-root migration · tray user label · the Account page eyeball | `specs/users-and-accounts.md` | UA-6 integration (worktree agent in flight) → UA-3w Setup Assistant → **UA-4 self/guest door (gates ANY shared-tailnet login; S1 live probe + two-account witness)** → UA-5 second user |
